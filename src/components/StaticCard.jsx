export default function StaticCard({ number }) {
  return (
    <div className="relative w-[70px] h-[67px] md:w-[148px] md:h-35">
      <div className="absolute inset-0 top-2 bg-shadow rounded-sm translate-y-2.5"></div>
      <div className="relative flex flex-col w-full h-full text-4xl md:text-[79px] font-red-hat font-bold text-core -tracking-[1px]">
        <div className="relative h-1/2 bg-card-top rounded-t-sm border-b border-shadow overflow-hidden">
          <span className="absolute left-1/2 top-full -translate-x-1/2 -translate-y-1/2">
            {number}
          </span>
        </div>
        <div className="relative h-1/2 bg-card-bot rounded-b-sm overflow-hidden shadow-inner">
